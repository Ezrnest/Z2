...
void GameUtil::saveGame(QWidget* parent,const shared_ptr<World> &world)
{
    ...
    QString path = QFileDialog::getSaveFileName(parent,"保存游戏", recommandedFileName,"*.save");
    if(path.isEmpty()){
        return;
    }
    ...
    if(!re){
        QMessageBox::warning(parent,"警告","保存游戏失败！");
    }
}
...
{
    string s = z2::GameConfiguration::instance().getSaveDir().getPath();
    QString path = QFileDialog::getOpenFileName(parent,"加载存档",QString::fromStdString(s),"*.save");
    if(path.isEmpty()){
        return shared_ptr<GameMap>();
    }
    auto map = GameMap::loadWorldFrom(path.toStdString());
    if(!map){
        QMessageBox::warning(parent,"警告","加载游戏失败！");
        return shared_ptr<GameMap>();
    }
    return map;
}
